...
The study of $\NP$ optimization problems has existed as long as computational complexity theory.
\emph{TODO Is there a history in Complexity and Approximation?}
The definitions we use are inspired by those in the 1999 book \emph{Complexity and Approximation} by Ausiello et al.
...
??? proved that the ??? problem is complete for $\NPO$; this guides our proofs in \autoref{sec:nncocomplete} and \autoref{sec:nncopocomplete} below.
For $\APX$-completeness, there are two known ways for showing a generic reduction from any $\APX$ problem: showing a reduction to ??? using the PCP Theorem as in the work of ??? in ??? or showing a reduction to a canonical, ``universal'' complete problem as in ???.
