...
The converse does not hold: as a counterexample, the minimum vertex cover problem is not in $\ENCAS$ (since no polynomial-time approximation algorithm with approximation ratio better than \sfrac{\textpl{7}\kern-0.3em}{\kern-0.3em\textpl{6}} exists \autocite[Theorem~8.1]{hastad01}) but the problem is in $\para \NC$ \autocite[Theorem~4.5]{bst15}.
